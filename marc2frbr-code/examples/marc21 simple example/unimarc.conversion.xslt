...
<xsl:stylesheet xmlns:local="http://idi.ntnu.no/frbrizer/" xmlns:f="http://idi.ntnu.no/frbrizer/"
   ...
   <xsl:template name="MARC21-001-Manifestation">
      ...
      <xsl:for-each select="node()[@*:tag='001']">
         ...
         <xsl:element name="{name(ancestor-or-self::*:record)}"
            ...
            <xsl:attribute name="f:type" select="'http://iflastandards.info/ns/fr/frbr/frbrer/C1003'"/>
            <xsl:if test="$include_labels">
               <xsl:attribute name="f:label" select="'Manifestation'"/>
            </xsl:if>
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='010'][*:subfield/@*:code = ('a')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='021'][*:subfield/@*:code = ('a','b')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
                  </xsl:if>
                  <xsl:for-each select="*:subfield[@*:code = ('a','b')]">
                     <xsl:if test="@*:code = 'a'">
                        <xsl:copy-of select="."/>
                     </xsl:if>
                     <xsl:if test="@*:code = 'b'">
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='200'][*:subfield/@*:code = ('a')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='205'][*:subfield/@*:code = ('a')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='210'][*:subfield/@*:code = ('a','c','d')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
                  </xsl:if>
                  <xsl:for-each select="*:subfield[@*:code = ('a','c','d')]">
                     <xsl:if test="@*:code = 'a'">
                        <xsl:copy-of select="."/>
                     </xsl:if>
                     <xsl:if test="@*:code = 'c'">
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='215'][*:subfield/@*:code = ('a','d')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='225'][*:subfield/@*:code = ('a','v')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
   ...
   <xsl:template name="MARC21-200-Work">
      ...
      <xsl:for-each select="node()[@*:tag='200']">
         ...
         <xsl:element name="{name(ancestor-or-self::*:record)}"
            ...
            <xsl:for-each select="$record/node()[@*:tag='200'][@ind1 = '1']">
               ...
               <xsl:if test="($target_field = $this_field)">
                  <f:relationship>
                     <xsl:attribute name="f:type" select="'http://iflastandards.info/ns/fr/frbr/frbrer/P2001'"/>
                     <xsl:if test="$include_labels">
                        <xsl:attribute name="f:label" select="'is realized through'"/>
                     </xsl:if>
                     <xsl:attribute name="f:itype" select="'http://iflastandards.info/ns/fr/frbr/frbrer/P2002'"/>
                     <xsl:if test="$include_labels">
                        <xsl:attribute name="f:ilabel" select="'is realization of'"/>
                     </xsl:if>
                     <xsl:if test="$include_target_entity_type">
                        <xsl:attribute name="f:target_type"
                                       select="'http://iflastandards.info/ns/fr/frbr/frbrer/C1002'"/>
                     </xsl:if>
                     <xsl:if test="$include_counters">
                        <xsl:attribute name="f:c" select="1"/>
                     </xsl:if>
   ...
   <xsl:template name="MARC21-240-Expression">
      ...
      <xsl:variable name="record" select="."/>
      <xsl:variable name="marcid" select="*:controlfield[@*:tag='001']"/>
      <xsl:for-each select="node()[@*:tag='200'][@ind1 = '1']">
         <xsl:variable name="this_field"
                       select="(ancestor-or-self::*:datafield, ancestor-or-self::*:controlfield)"/>
         <xsl:variable name="anchor_field"
                       select="(ancestor-or-self::*:datafield, ancestor-or-self::*:controlfield)"/>
         ...
         <xsl:element name="{name(ancestor-or-self::*:record)}"
            ...
            <xsl:attribute name="f:id"
                           select="string-join(($record/@*:id,$this_template_name,$tag,$this_field_position), ':')"/>
            <xsl:attribute name="f:type" select="'http://iflastandards.info/ns/fr/frbr/frbrer/C1002'"/>
            <xsl:attribute name="f:subtype"
                           select="($anchor_field/*:subfield[@code='h'], $record/*:datafield[@tag='336']/*:subfield[@code = 'a'])[1]"/>
            <xsl:if test="$include_labels">
               <xsl:attribute name="f:label" select="'Expression'"/>
            </xsl:if>
            <xsl:attribute name="f:templatename" select="$this_template_name"/>
            <xsl:if test="$include_counters">
               <xsl:attribute name="f:c" select="1"/>
            ...
            <xsl:for-each select="$record/*:datafield[@*:tag='200'][. = $this_field][*:subfield/@*:code = ('a','d')]">
               <xsl:copy>
                  <xsl:call-template name="copy-attributes"/>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
                  ...
                  <xsl:for-each select="*:subfield[@*:code = ('a','d')]">
                     <xsl:if test="@*:code = 'a'">
                        <xsl:copy>
                           <xsl:call-template name="copy-content">
                              <xsl:with-param name="type" select="'http://iflastandards.info/ns/fr/frbr/frbrer/P3008'"/>
                              <xsl:with-param name="select" select="."/>
                           </xsl:call-template>
                        </xsl:copy>
                     </xsl:if>
               ...
               <xsl:copy>
                  ...
                  <xsl:for-each select="*:subfield[@*:code = ('a')]">
                     <xsl:if test="@*:code = 'a'">
                        <xsl:copy>
                           <xsl:call-template name="copy-content">
                              <xsl:with-param name="type" select="'http://iflastandards.info/ns/fr/frbr/frbrer/P3058'"/>
                              <xsl:with-param name="select" select="."/>
                           </xsl:call-template>
                        </xsl:copy>
                     </xsl:if>
            ...
            <xsl:for-each select="$record/node()[@*:tag='001']">
               ...
               <xsl:variable name="target_field_position" as="xs:string" select="string(position())"/>
               <f:relationship>
                  <xsl:attribute name="f:type" select="'http://iflastandards.info/ns/fr/frbr/frbrer/P2003'"/>
                  <xsl:attribute name="f:itype" select="'http://iflastandards.info/ns/fr/frbr/frbrer/P2004'"/>
                  <xsl:if test="$include_target_entity_type">
                     <xsl:attribute name="f:target_type"
                                    select="'http://iflastandards.info/ns/fr/frbr/frbrer/C1003'"/>
                  </xsl:if>
                  <xsl:if test="$include_counters">
                     <xsl:attribute name="f:c" select="1"/>
                  </xsl:if>
         ...
         <xsl:for-each select="*:record">
            <xsl:choose>
               ...
               <xsl:when test="@*:templatename = 'MARC21-001-Manifestation'">
                  <xsl:element name="f:keyentry">
                     <xsl:variable name="key">
                        <xsl:value-of select="local:sort-keys(if (*:datafield[@tag=&#34;010&#34;][1]/*:subfield[@code='a'][1]) then *:datafield[@tag=&#34;010&#34;][1]/*:subfield[@code='a'][1] else *:controlfield[@tag='001'])"/>
                        <xsl:value-of select="concat('\',@*:type, '#')"/>
                     </xsl:variable>
                     <xsl:variable name="keyvalue"
                                   select="replace(lower-case(string-join($key, '\')), '[^a-z0-9\\#|¤§]', '')"/>
   ...
   <xsl:template match="f:record-set" mode="create-key-mapping-step-3">
      <f:keymap>
         <xsl:for-each select="*:record">
            <xsl:choose>
               <xsl:when test="@*:templatename = 'MARC21-240-Expression'">
                  <xsl:element name="f:keyentry">
                     <xsl:variable name="key">
                        <xsl:value-of select="local:sort-keys(*:relationship[ends-with(@*:type, 'P2002')][1]/@*:href)"/>
                        <xsl:value-of select="local:sort-keys(*:datafield[@tag = '205']/*:subfield[@code='a'])"/>
                        <xsl:value-of select="concat('\',@*:type, '#')"/>
                     </xsl:variable>
                     <xsl:variable name="keyvalue"
                                   select="replace(lower-case(string-join($key, '\')), '[^a-z0-9\\#|¤§]', '')"/>
               ...
               <xsl:choose>
                  ...
                  <xsl:when test="@*:templatename = 'MARC21-240-Expression'">
                     <xsl:element name="f:label">
                        ...
                        <xsl:value-of select="string-join((*:datafield/*:subfield[ends-with(@*:type, 'P3009')][1], *:datafield/*:subfield[ends-with(@*:type, 'P3011')][1], *:datafield/*:subfield[ends-with(@*:type, 'P3008')]), ' / ')"/>
                     </xsl:element>
                  </xsl:when>
                  <xsl:when test="@*:templatename = 'MARC21-001-Manifestation'">
                     <xsl:element name="f:label">
                        ...
                        <xsl:value-of select="string-join(*:datafield[@tag = '245']/*:subfield[@code = ('a', 'b', 'c')], ' / ')"/>
                     </xsl:element>
                  </xsl:when>
               </xsl:choose>
            </xsl:copy>
